...
interface TaskItem {
  ...
  repeatType: 'none' | 'weekly' | 'monthly'; // 繰り返しタイプ
  status: '未着手' | '進行中' | '完了';
  dueDate: string; // 期限・実施日
  assignee: string; // 担当者
  content: string; // 議事録・メモ詳細
  createdAt: string;
}

export default function Home() {
  // タスク・議事録一覧State（初期サンプルデータ）
  ...
  return (
    <main className="p-6 max-w-7xl mx-auto space-y-8 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900">タスク ＆ 議事録管理</h1>
          <p className="text-xs text-gray-500 mt-1">週・月ごとの繰り返し業務の自動作成 ＆ 進捗ダッシュボード</p>
        </div>
      </div>
