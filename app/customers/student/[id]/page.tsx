...
// Supabaseクライアントの初期化（Vercelの環境変数を使用）
    ...
    async function fetchCustomerData() {
      try {
        ...
        if (data && data.length > 0) {
          // 取得できたデータをセット（存在しない項目はデフォルト値を補完）
          const dbCustomer = data[0];
          setStudent({
            id: dbCustomer.id || studentId,
            name: dbCustomer.name || '名前未設定',
            kana: dbCustomer.kana || 'カナ未設定',
            birthdate: dbCustomer.birthdate || '2015-05-12',
            firstLessonDate: dbCustomer.first_lesson_date || '2026-03-01',
            lastReservationDate: dbCustomer.last_reservation_date || '2026-08-10',
            concern: dbCustomer.concern || '特になし',
            target: dbCustomer.target || '目標未設定',
            memo: dbCustomer.memo || ''
          });
        }
      } catch (err) {
    ...
    return (
      <div className="p-6 bg-slate-50 min-h-screen flex items-center justify-center">
        <p className="text-slate-700 font-bold">
          生徒データが見つかりません。Supabaseの `customers` テーブルにデータを追加してください。
        </p>
      </div>
    );
  }
