        ...
        <div className="w-full md:w-80 bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col gap-4 shadow-lg">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 max-h-[calc(100vh-280px)]">
            ...
            {filteredStudents.length === 0 && (
              <div className="text-center py-8 text-slate-500 text-sm">該当する受講生がいません</div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col gap-6 shadow-lg">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end">
              <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-bold transition ${
                    activeTab === 'edit_info' ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  基本情報編集
                </button>
          ...
          {activeTab === 'carte' && (
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-950/60 p-4 rounded-xl border border-slate-800">
                <div>
                  <span className="text-xs text-slate-400 block font-bold mb-1">🎯 お悩み・課題</span>
                  <p className="text-sm text-slate-200">{currentStudent.concern}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-400 block font-bold mb-1">⭐ 目標</span>
                  <p className="text-sm text-slate-200">{currentStudent.target}</p>
                </div>
              </div>

              {/* 📸 写真比較セクション (姿勢・ケガゼロ・フィジカルチェック) */}
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <div className="flex items-center gap-1">
                      <span className="text-slate-400 font-bold">比較: 過去(Before)</span>
                      <select
                        value={beforeDate}
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-slate-400 font-bold">最新(After)</span>
                      <select
                        value={afterDate}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 flex flex-col gap-2">
                      <div className="flex justify-between items-center text-xs">
                        <span className="font-bold text-indigo-400">Before ({beforeDate})</span>
                      ...
                      <div className="flex flex-wrap gap-2 min-h-[80px] items-center">
                        {beforePhysical?.kegazeroPhotos && beforePhysical.kegazeroPhotos.length > 0 ? (
                          beforePhysical.kegazeroPhotos.map((photo, idx) => (
                            <div key={`b-kegazero-${idx}`} className="relative w-20 h-20 bg-slate-950 border border-slate-800 rounded overflow-hidden group">
                              <img
                                src={photo}
                          ...
                          <span className="text-xs text-slate-500 italic w-full text-center py-2">写真なし</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 flex flex-col gap-2">
                      <div className="flex justify-between items-center text-xs">
                        <span className="font-bold text-emerald-400">After ({afterDate})</span>
                      ...
                      <div className="flex flex-wrap gap-2 min-h-[80px] items-center">
                        {afterPhysical?.kegazeroPhotos && afterPhysical.kegazeroPhotos.length > 0 ? (
                          afterPhysical.kegazeroPhotos.map((photo, idx) => (
                            <div key={`a-kegazero-${idx}`} className="relative w-20 h-20 bg-slate-950 border border-slate-800 rounded overflow-hidden group">
                              <img
                                src={photo}
                      ...
                      <div className="flex flex-wrap gap-2 min-h-[80px] items-center">
                        {beforePhysical?.physicalCheckPhotos && beforePhysical.physicalCheckPhotos.length > 0 ? (
                          beforePhysical.physicalCheckPhotos.map((photo, idx) => (
                            <div key={`b-pcheck-${idx}`} className="relative w-20 h-20 bg-slate-950 border border-slate-800 rounded overflow-hidden group">
                              <img
                                src={photo}
                      ...
                      <div className="flex flex-wrap gap-2 min-h-[80px] items-center">
                        {afterPhysical?.physicalCheckPhotos && afterPhysical.physicalCheckPhotos.length > 0 ? (
                          afterPhysical.physicalCheckPhotos.map((photo, idx) => (
                            <div key={`a-pcheck-${idx}`} className="relative w-20 h-20 bg-slate-950 border border-slate-800 rounded overflow-hidden group">
                              <img
                                src={photo}
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2 w-full md:w-auto">
                  <span className="text-xs font-bold text-slate-300 whitespace-nowrap">📅 新規測定日を追加:</span>
                  <input
                    type="date"
                ...
                <button
                  ...
                  className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-4 py-2 rounded-lg transition shadow"
                >
                  ＋ この日付の計測データ・写真枠を追加
                </button>
              </div>
              ...
              {/* セッション・レッスン記録追加セクション */}
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 flex flex-col gap-4">
                <h3 className="font-bold text-sm text-emerald-400">📝 新規セッション・レッスン記録の追加</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    <label className="text-[11px] text-slate-400 block mb-1">実施日</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="text-[11px] text-slate-400 block mb-1">担当トレーナー</label>
                    <select
                  ...
                  <div>
                    <label className="text-[11px] text-slate-400 block mb-1">宿題・自主トレ</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-[11px] text-slate-400 block mb-1">レッスン内容・フィジカルメモ</label>
                  <textarea
                    rows={2}
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-5 py-2 rounded-lg transition shadow"
                  >
                    セッション記録を登録する
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-3">
                <h3 className="font-bold text-sm text-slate-300">📜 過去のセッション履歴</h3>
                <div className="space-y-3">
                  {currentStudent.sessions.map(ses => (
                    <div key={ses.id} className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-4 flex flex-col gap-2">
                      <div className="flex justify-between items-center text-xs border-b border-slate-800/60 pb-2">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-emerald-400">{ses.date}</span>
                          <span className="bg-slate-800 text-slate-300 px-2 py-0.5 rounded text-[10px]">担当: {ses.staff}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                      ...
                      <p className="text-sm text-slate-200 whitespace-pre-wrap">{ses.content}</p>
                      {ses.homework && (
                        <div className="text-xs bg-slate-900 border border-slate-800/50 p-2 rounded text-indigo-300">
                  ...
                  {currentStudent.sessions.length === 0 && (
                    <div className="text-center py-6 text-slate-500 text-xs">セッション記録はまだありません</div>
                  )}
                </div>
              </div>
            </div>
          ...
          {activeTab === 'tickets' && (
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-base text-emerald-400">🎟️ チケット・購入履歴</h3>
                <div className="text-xs text-slate-300">
                  現在保有残高: <strong className="text-emerald-400 text-base">{currentParent.ticketRemaining}回</strong>
                </div>
              </div>

              <div className="space-y-2">
                {currentParent.ticketsHistory.map(th => (
                  <div key={th.id} className="bg-slate-950 border border-slate-800 rounded-lg p-3 flex justify-between items-center">
                    <div>
                      <div className="font-bold text-sm text-white">{th.title}</div>
                      <div className="text-xs text-slate-400 mt-1">購入日: {th.date} / 有効期限: {th.expire}</div>
                    </div>
                    <div className="text-right">
                      <span className="text-xs bg-emerald-950 text-emerald-400 px-2.5 py-1 rounded border border-emerald-800 font-bold">
                        +{th.count}回付与
                      </span>
                      <div className="text-[10px] text-slate-500 mt-1">ID: {th.squarePaymentId}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'edit_info' && (
            <div className="flex flex-col gap-4 max-w-lg">
              <h3 className="font-bold text-base text-emerald-400">✏️ 生徒・保護者基本情報の編集</h3>
              
              <div className="space-y-3">
                <div>
                  <label className="text-xs text-slate-400 block mb-1">受講生のお名前</label>
                  <input
                    type="text"
                    value={editForm.name}
                ...
                <div>
                  <label className="text-xs text-slate-400 block mb-1">フリガナ</label>
                  <input
                    type="text"
                    value={editForm.kana}
                ...
                <div>
                  <label className="text-xs text-slate-400 block mb-1">保護者連絡先 (電話番号)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs text-slate-400 block mb-1">お悩み・課題</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs text-slate-400 block mb-1">目標</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs text-slate-400 block mb-1">メモ・既往歴等</label>
                  <textarea
                    rows={3}
                    value={editForm.memo}
      ...
      {editingSession && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full flex flex-col gap-4 shadow-2xl">
            <h3 className="font-bold text-base text-emerald-400">✏️ セッション記録の編集</h3>
            <div>
              <label className="text-xs text-slate-400 block mb-1">実施日</label>
              <input
                type="date"
            ...
            <div>
              <label className="text-xs text-slate-400 block mb-1">担当</label>
              <select
            ...
            <div>
              <label className="text-xs text-slate-400 block mb-1">内容</label>
              <textarea
                rows={3}
            ...
            <div>
              <label className="text-xs text-slate-400 block mb-1">宿題</label>
              <input
                type="text"
