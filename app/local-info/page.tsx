...
// 学校・スポーツクラブ情報の型定義
interface SchoolOrClubInfo {
  id: string;
  name: string; // 学校名またはチーム名
  category: '所属チーム' | '学校・学童' | '外部スクール';
  gradeOrClass: string; // 学年・クラス・カテゴリー（例: 小5 / U-11）
  coachOrContact: string; // 指導者名や連絡先等
  description: string; // 特記事項・練習曜日やプレースタイルなど
}
...
export default function ClientsPage() {
  ...
  // 学校・スポーツクラブデータ
  const [schoolClubInfos, setSchoolClubInfos] = useState<SchoolOrClubInfo[]>([
    {
      id: 'sc-1',
      name: 'FCビクトリー練馬',
      category: '所属チーム',
      gradeOrClass: 'U-12 (小学6年生)',
      coachOrContact: '鈴木コーチ (090-XXXX-YYYY)',
      description: '週末は土日とも練習・試合。ポジションはボランチ。ハードワークを求められるチーム。'
    },
    {
      id: 'sc-2',
      name: '練馬区立光が丘小学校',
      category: '学校・学童',
      gradeOrClass: '6年2組',
      coachOrContact: '担任: 佐藤先生',
      description: '放課後は校庭開放やサッカーの自主練を実施。'
    }
  ]);
  ...
  const [students, setStudents] = useState<Student[]>([
    {
      ...
      physicalHistory: [
        { id: 'm-1', date: '2026-06-01', weight: 37.5, fat: 16.0, muscle: 29.5, note: '初回3ヶ月測定', posturePhotos: { front: null, side: null, back: null }, testPhotos: [] },
        { id: 'm-2', date: '2026-09-01', weight: 38.7, fat: 15.2, muscle: 30.8, note: '2回目3ヶ月測定', posturePhotos: { front: null, side: null, back: null }, testPhotos: [] }
      ],
      sessions: [
    ...
    },
    {
      ...
      physicalHistory: [
        { id: 'm-3', date: '2026-07-01', weight: 25.0, fat: 14.5, muscle: 18.0, note: '初回計測', posturePhotos: { front: null, side: null, back: null }, testPhotos: [] }
      ],
      sessions: [
  ...
  const [activeTab, setActiveTab] = useState<'carte' | 'tickets' | 'school_club' | 'edit_info'>('carte');
  ...
  // 学校・スポーツクラブ追加・編集用ステート
  const [newScName, setNewScName] = useState('');
  const [newScCategory, setNewScCategory] = useState<'所属チーム' | '学校・学童' | '外部スクール'>('所属チーム');
  const [newScGrade, setNewScGrade] = useState('');
  const [newScCoach, setNewScCoach] = useState('');
  ...
  const [editScGrade, setEditScGrade] = useState('');
  ...
  const getAlertBadges = (student: Student, parent: Parent) => {
    ...
    if (student.lastReservationDate) {
      ...
      if (diffDays >= 30) {
        ...
        alerts.push({ text: `⚠️ 2週間未予約 (${diffDays}日前)`, type: 'warning' });
      }
    }
  ...
  // 学校・スポーツクラブ追加・編集・削除ハンドラー
  const handleAddSchoolClub = () => {
    if (!newScName) {
      alert('名称を入力してください。');
      return;
    }
    const newItem: SchoolOrClubInfo = {
      id: `sc-${Date.now()}`,
      name: newScName,
      category: newScCategory,
      gradeOrClass: newScGrade,
      coachOrContact: newScCoach,
      description: newScDesc
    };
    setSchoolClubInfos([newItem, ...schoolClubInfos]);
    setNewScName('');
    setNewScGrade('');
    setNewScCoach('');
    setNewScDesc('');
    alert('学校・スポーツクラブ情報を追加しました！');
  };

  const handleDeleteSchoolClub = (id: string) => {
    if (!confirm('この情報を削除しますか？')) return;
    setSchoolClubInfos(prev => prev.filter(item => item.id !== id));
  };

  const handleSaveEditSchoolClub = (id: string) => {
    setSchoolClubInfos(prev =>
      prev.map(item => (item.id === id ? { ...item, name: editScName, gradeOrClass: editScGrade, description: editScDesc } : item))
    );
    setEditingScId(null);
    alert('情報を更新しました！');
  };
          ...
          <div className="md:col-span-3 space-y-5">
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
              ...
              <div className="flex flex-wrap border-b border-slate-200 pt-2 gap-6 text-xs font-bold">
                ...
                <button onClick={() => setActiveTab('school_club')} className={`pb-3 border-b-2 transition ${activeTab === 'school_club' ? 'border-[#5e9bc4] text-[#5e9bc4]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
                  ⚽ 学校・所属クラブ管理
                </button>
            ...
            {/* TAB 3: 学校・所属スポーツクラブ管理（新規追加・編集・削除対応） */}
            {activeTab === 'school_club' && (
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-5">
                <div className="border-b pb-3">
                  <h3 className="font-bold text-slate-800 text-sm flex items-center gap-1.5"><span>⚽</span> 学校・所属スポーツクラブ情報</h3>
                  <p className="text-xs text-slate-400 mt-0.5">受講生が通う学校、所属チーム（サッカー少年団・クラブチーム）、外部スクールなどを管理できます。</p>
                </div>

                {/* 新規追加フォーム */}
                <div className="bg-sky-50/60 p-4 rounded-xl border border-sky-100 space-y-3 text-xs">
                  <h4 className="font-bold text-[#5e9bc4]">＋ 学校・チーム情報の追加</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input type="text" placeholder="名称（例: FCビクトリー）" value={newScName} onChange={e => setNewScName(e.target.value)} className="border rounded p-2 bg-white" />
                    <select value={newScCategory} onChange={e => setNewScCategory(e.target.value as any)} className="border rounded p-2 bg-white font-bold text-slate-700">
                      <option value="所属チーム">所属チーム</option>
                      <option value="学校・学童">学校・学童</option>
                      <option value="外部スクール">外部スクール</option>
                    </select>
                    <input type="text" placeholder="学年・カテゴリー（例: U-12）" value={newScGrade} onChange={e => setNewScGrade(e.target.value)} className="border rounded p-2 bg-white" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input type="text" placeholder="指導者・連絡先（例: 鈴木コーチ）" value={newScCoach} onChange={e => setNewScCoach(e.target.value)} className="border rounded p-2 bg-white" />
                    <input type="text" placeholder="備考（例: 土日は試合中心）" value={newScDesc} onChange={e => setNewScDesc(e.target.value)} className="border rounded p-2 bg-white" />
                  </div>
                  <button onClick={handleAddSchoolClub} className="bg-[#5e9bc4] hover:bg-sky-600 text-white font-bold px-4 py-2 rounded shadow-sm transition">
                    登録する
                  </button>
                </div>
                ...
                {/* 一覧・編集・削除 */}
                <div className="space-y-3">
                  {schoolClubInfos.map(item => (
                    <div key={item.id} className="p-4 bg-slate-50 border rounded-xl text-xs space-y-2">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded font-bold text-[10px] ${item.category === '所属チーム' ? 'bg-emerald-100 text-emerald-800' : 'bg-sky-100 text-[#5e9bc4]'}`}>
                            {item.category}
                          </span>
                          <span className="font-bold text-slate-700">{item.gradeOrClass}</span>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => { setEditingScId(item.id); setEditScName(item.name); setEditScGrade(item.gradeOrClass); setEditScDesc(item.description); }} className="text-sky-600 hover:underline">編集</button>
                          <button onClick={() => handleDeleteSchoolClub(item.id)} className="text-rose-500 hover:underline">削除</button>
                        </div>
                      </div>

                      {editingScId === item.id ? (
                        <div className="space-y-2 pt-2 border-t">
                          <input type="text" value={editScName} onChange={e => setEditScName(e.target.value)} className="w-full border rounded p-1 bg-white" placeholder="名称" />
                          <input type="text" value={editScGrade} onChange={e => setEditScGrade(e.target.value)} className="w-full border rounded p-1 bg-white" placeholder="学年・カテゴリー" />
                          <input type="text" value={editScDesc} onChange={e => setEditScDesc(e.target.value)} className="w-full border rounded p-1 bg-white" placeholder="備考" />
                          <div className="flex gap-2 justify-end">
                            <button onClick={() => handleSaveEditSchoolClub(item.id)} className="bg-emerald-600 text-white px-3 py-1 rounded font-bold">保存</button>
                            <button onClick={() => setEditingScId(null)} className="bg-slate-300 px-3 py-1 rounded">キャンセル</button>
                          </div>
                        </div>
                      ) : (
                        <div>
                          <h4 className="font-bold text-slate-900 text-sm mt-1">{item.name}</h4>
                          <p className="text-slate-500">👤 指導者・連絡先: {item.coachOrContact}</p>
